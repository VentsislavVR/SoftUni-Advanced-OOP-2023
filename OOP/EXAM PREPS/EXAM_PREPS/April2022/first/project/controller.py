...
class Controller():
    ...
    def sustain(self,player_name: str, sustenance_type: str)-> str or None:
        if sustenance_type not in self.VALID_TYPE:
            return
        ...
        for i in range(len(self.supplies)-1,-1,-1):
            ...
            if supply.__class__.name == sustenance_type:
                self.supplies.pop(i)
                break
        else:
    ...
    def next_day(self)->None:
        for player in self.players:
            ...
            player.stamina = max(reduce_stamina_with,0)
    ...
    def __str__(self):
        return "\n".join(
            ...
            +
            [sup.details() for sup in self.supplies]
